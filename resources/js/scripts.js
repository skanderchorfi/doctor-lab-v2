if (document.getElementById('chord-diagram')) {

    // recuperer les données chord depuis la base
    axios.get('http://localhost:8000/api/chord/data')
        .then(response => {
            const { matrix, nameProvider } = response.data
            let NameProvider = nameProvider

            var colors = ["#C4C4C4","#69B40F","#EC1D25","#C8125C","#008FC8","#10218B","#134B24","#737373","33FFDA"];//pour chaque article ->couleur spécifique

            /*Initiate the color scale*/
            var fill = d3.scale.ordinal()//construire une nouvelle echelle
                .domain(d3.range(NameProvider.length))//tableau des produits
                .range(colors);
            ...
            var margin = {top: 30, right: 25, bottom: 20, left: 25},
                ...
                outerRadius = innerRadius * 1.04;//couleur talwin diagraam//dégradation couleur de diag

            /*Initiate the SVG*///balise html image victorielle
//papier dessin//
            var svg = d3.select("#chart").append("svg:svg")//selectioner l'element (id chart) //append ajouter un element svg
                .attr("width", width + margin.left + margin.right)
                .attr("height", height + margin.top + margin.bottom)
                .append("svg:g")//ajouter une balise g
                .attr("transform", "translate(" + (margin.left + width/2) + "," + (margin.top + height/2) + ")");//#id .class

///////*donnez cette matrice à d3.chord (): il calculera toutes les informations dont nous avons besoin pour dessiner arc et ruban*/////////
            var chord = d3.layout.chord()
                .padding(.04)
                .sortSubgroups(d3.descending) /*trier les accords à l'intérieur d'un arc de haut en bas*/
                .sortChords(d3.descending) /*
quel accord doit être affiché en haut lorsque les accords se croisent. Maintenant, le plus gros accord est en bas*/
                .matrix(matrix);
...
//creation arc//
            var arc = d3.svg.arc()
                ...
                .outerRadius(outerRadius);//les ongles de l'arc//


            var g = svg.selectAll("g.group")//selectioner les elements g li andhom class group
                .data(chord.groups)
                .enter().append("svg:g")
                .attr("class", function(d) {return "group " + NameProvider[d.index];});//affichage les noms du group//aple groupe//

            g.append("svg:path")//ajouter un chemin vectiorielle ->svg
                .attr("class", "arc")
                .style("stroke", function(d) { return fill(d.index); })//stroke -
                .style("fill", function(d) { return fill(d.index); })//fill fi weset l'arc
                .attr("d", arc)//object
                .style("opacity", 0)
                .transition().duration(1000)//annimation
                .style("opacity", 0.4);
...
//les noms du groupe//
            ...
            var middleTextTop = textCenter.append("text")
                ...
                .attr("dy", "1em")
                .attr("opacity", 1)
                .text("Nous avons collecter des données des patients qui souffre de diabéte")
                .call(wrap, 350);
            ...
            var middleTextBottom = textCenter.append("text")
                ...
                .attr("dy", "1em")
                .attr('opacity', 1)
                .text("Les répondants nous ont donné des informations sur le diabéte par rapport au temps , taux de glycémie et la position actuelles de chaque patient ")
                .call(wrap, 350);
            ...
            var counter = 1,
                buttonTexts = ["D'accord","Continue","Continue","Continue","Continue","Continue","Continue","Continue",
                    "Continue","Continue","Continue","Continue","Continue","Finir"],
            ...
            function Draw1(){
                ...
                changeTopText(newText = "De nos jours,La plupart des gens souffrent de diabète  dans ces dernières années " +
                    "Certaines personnes restent  à leur état, mais beaucoup passent à d'autre type de diabéte ...",
                    loc = 4/2, delayDisappear = 0, delayAppear = 1);

                changeTopText(newText = "Dans les prochaines étapes, je voudrais vous présenter les flux de personnes entre le taux de glycémie,temps et position ",
                    loc = 8/2, delayDisappear = 9, delayAppear = 10, finalText = true);

                changeBottomText(newText = "Commençons par dessiner la division des plusieurs personnes, qui ont eu un taux de glécimie avec ses différents classement",
            ...
            function Draw2(){
                ...
                /*Switch  text*/
                changeTopText(newText = "Selon l'enquête, % ",
